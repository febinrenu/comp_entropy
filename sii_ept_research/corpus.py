# -*- coding: utf-8 -*-
"""
60-prompt corpus, 12 per domain, constructed for this pilot replication
(the original 60-prompt corpus used in the manuscript was not available,
so this corpus is author-constructed to match the described domain
distribution: factual QA, instruction following, summarisation,
code explanation, creative writing).
"""
...
SUMMARISATION = [
    ...
    "Summarise a news story about a small business that successfully transitioned to an online-only sales model during a economic downturn.",
...
for domain, prompts in [
    ...
    assert len(prompts) == 12, f"{domain} has {len(prompts)} prompts"
    for i, p in enumerate(prompts):
        CORPUS.append({"id": f"{domain}_{i+1:02d}", "domain": domain, "text": p})
